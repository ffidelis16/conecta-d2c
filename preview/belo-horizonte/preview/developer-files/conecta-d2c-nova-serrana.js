/* Conecta D2C Belo Horizonte | prévia local de trabalho · sem formulário conectado */
(function conectaD2C() {
  "use strict";
  ...
  const formConfig = {
    ...
    event_pre_registration: {
      ...
      title: "Confirme seu interesse",
      description: "O formulário desta edição será conectado após a definição da configuração final. A confirmação de presença seguirá os critérios e a capacidade definidos para Belo Horizonte.",
    },
    event_registration: {
  ...
  const renderForm = async (type) => {
    ...
    if (!config.id) {
      ...
      pending.textContent = "O formulário será disponibilizado após a confirmação da configuração da edição.";
